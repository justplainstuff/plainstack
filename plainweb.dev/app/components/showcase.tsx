...
`;
  ...
  },
  {
    title: "Tasks",
    content: async () => {
      const code = `// app/tasks/double-opt-in.ts
      
export default createTask({
  async fetch() {
    return await database.query.contacts.findMany({
      where: (contacts, { isNull }) => isNull(contacts.doubleOptSent),
      limit: 5,
    });
  },
  async process(contact: Contact) {
    await sendDoubleOptInEmail(database, contact);
  },
});
